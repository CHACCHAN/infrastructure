...
- インベントリに無いVMを変数だけで構築する方法は [docs/pve/adhoc.md](../pve/adhoc.md)、AWXから実行する前提は [docs/utils/awx/](../utils/awx/README.md)
...
| [coolify.yml](../../playbooks/vm/coolify.yml) | [coolify.md](coolify.md) | Coolify(セルフホストPaaS) | − |
...
| 構築は成功したのに最後の疎通確認だけ失敗 | 確認は実行元(AWXなら実行Pod)から行われる。実行元→VMのLAN側IPへの経路・firewallを確認([docs/utils/awx/](../utils/awx/README.md)) |
